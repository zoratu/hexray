...
use crate::signature::{CallingConvention, FunctionSignature, Parameter, ParameterType};
...
fn add_string_functions(db: &mut SignatureDatabase) {
    ...
    if let Ok(sig) =
        FunctionSignature::from_hex("__strlen_sse2", "F3 0F 1E FA 89 F8 25 ?? ?? ?? ?? 48")
    {
        db.add(
    ...
    if let Ok(sig) = FunctionSignature::from_hex(
        "strncpy",
        "55 48 89 E5 48 83 EC 30 48 89 7D E8 48 89 75 E0 48 89 55 D8",
    ) {
        db.add(
            sig.with_convention(CallingConvention::SystemV)
                .with_return_type(ParameterType::String)
    ...
    if let Ok(sig) = FunctionSignature::from_hex(
        "strncmp",
        "55 48 89 E5 48 83 EC 20 48 89 7D E8 48 89 75 E0 48 89 55 D8",
    ) {
        db.add(
            sig.with_convention(CallingConvention::SystemV)
                .with_return_type(ParameterType::Int {
    ...
    // strcat
    if let Ok(sig) =
        FunctionSignature::from_hex("strcat", "55 48 89 E5 48 89 7D F8 48 89 75 F0 48 8B 45 F8")
    {
        db.add(
            sig.with_convention(CallingConvention::SystemV)
                .with_return_type(ParameterType::String)
    ...
    // strrchr
    if let Ok(sig) =
        FunctionSignature::from_hex("strrchr", "55 48 89 E5 48 89 7D E8 89 75 E4 48 C7 45 F8 00")
    {
        db.add(
            sig.with_convention(CallingConvention::SystemV)
                .with_return_type(ParameterType::String)
    ...
    // strstr
    if let Ok(sig) =
        FunctionSignature::from_hex("strstr", "55 48 89 E5 48 83 EC 20 48 89 7D E8 48 89 75 E0")
    {
        db.add(
            sig.with_convention(CallingConvention::SystemV)
                .with_return_type(ParameterType::String)
...
fn add_memory_functions(db: &mut SignatureDatabase) {
    ...
    if let Ok(sig) =
        FunctionSignature::from_hex("memcpy", "55 48 89 E5 48 89 7D E8 48 89 75 E0 48 89 55 D8")
    {
        db.add(
            sig.with_convention(CallingConvention::SystemV)
    ...
    if let Ok(sig) = FunctionSignature::from_hex(
        "memmove",
        "55 48 89 E5 48 89 7D E8 48 89 75 E0 48 89 55 D8 48 8B 45 E8 48 3B 45 E0",
    ) {
        db.add(
            sig.with_convention(CallingConvention::SystemV)
                .with_return_type(ParameterType::OpaquePtr)
    ...
    if let Ok(sig) =
        FunctionSignature::from_hex("memset", "55 48 89 E5 48 89 7D D8 89 75 D4 48 89 55 C8")
    {
        db.add(
            sig.with_convention(CallingConvention::SystemV)
                .with_return_type(ParameterType::OpaquePtr)
    ...
    if let Ok(sig) =
        FunctionSignature::from_hex("__memset_erms", "48 89 F8 40 88 F0 48 89 D1 F3 AA C3")
    {
        db.add(
    ...
    // calloc
    if let Ok(sig) =
        FunctionSignature::from_hex("calloc", "55 48 89 E5 53 48 83 EC 18 48 89 FB 48 89 F0")
    {
        db.add(
            sig.with_convention(CallingConvention::SystemV)
                .with_return_type(ParameterType::OpaquePtr)
    ...
    if let Ok(sig) = FunctionSignature::from_hex(
        "realloc",
        "55 48 89 E5 53 48 83 EC 18 48 89 FB 48 89 F0 48 85 FF",
    ) {
        db.add(
            sig.with_convention(CallingConvention::SystemV)
                .with_return_type(ParameterType::OpaquePtr)
...
fn add_io_functions(db: &mut SignatureDatabase) {
    ...
    // fopen
    if let Ok(sig) =
        FunctionSignature::from_hex("fopen", "55 48 89 E5 48 83 EC 20 48 89 7D E8 48 89 75 E0")
    {
        db.add(
            sig.with_convention(CallingConvention::SystemV)
    ...
    if let Ok(sig) = FunctionSignature::from_hex(
        "fread",
        "55 48 89 E5 48 83 EC 30 48 89 7D E8 48 89 75 E0 48 89 55 D8 48 89 4D D0",
    ) {
        db.add(
            sig.with_convention(CallingConvention::SystemV)
                .with_return_type(ParameterType::Size)
...
fn add_process_functions(db: &mut SignatureDatabase) {
    ...
    if let Ok(sig) = FunctionSignature::from_hex(
        "setenv",
        "55 48 89 E5 48 83 EC 30 48 89 7D E8 48 89 75 E0 89 55 DC",
    ) {
        db.add(
            sig.with_convention(CallingConvention::SystemV)
                .with_return_type(ParameterType::Int {
