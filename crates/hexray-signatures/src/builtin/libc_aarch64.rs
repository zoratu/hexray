...
fn add_string_functions(db: &mut SignatureDatabase) {
    ...
    if let Ok(sig) = FunctionSignature::from_hex("strlen", "FD 7B BE A9 FD 03 00 91") {
        db.add(
            sig.with_convention(CallingConvention::Aarch64)
    ...
    if let Ok(sig) =
        FunctionSignature::from_hex("__strlen_asimd", "00 70 40 0D ?? ?? ?? ?? 00 ?? 00 4E")
    {
        db.add(
    ...
    // strcmp
    if let Ok(sig) =
        FunctionSignature::from_hex("strcmp", "FD 7B BE A9 FD 03 00 91 E0 ?? 00 F9 E1 ?? 00 F9")
    {
        db.add(
            sig.with_convention(CallingConvention::Aarch64)
    ...
    // strchr - simple loop
    if let Ok(sig) =
        FunctionSignature::from_hex("strchr", "FD 7B BE A9 FD 03 00 91 E0 ?? 00 F9 E1 0F 00 39")
    {
        db.add(
            sig.with_convention(CallingConvention::Aarch64)
                .with_return_type(ParameterType::String)
...
fn add_memory_functions(db: &mut SignatureDatabase) {
    ...
    if let Ok(sig) = FunctionSignature::from_hex(
        "memcpy",
        "FD 7B BE A9 FD 03 00 91 E0 ?? 00 F9 E1 ?? 00 F9 E2 ?? 00 F9",
    ) {
        db.add(
            sig.with_convention(CallingConvention::Aarch64)
    ...
    if let Ok(sig) = FunctionSignature::from_hex(
        "memset",
        "FD 7B BE A9 FD 03 00 91 E0 ?? 00 F9 E1 0F 00 39 E2 ?? 00 F9",
    ) {
        db.add(
            sig.with_convention(CallingConvention::Aarch64)
                .with_return_type(ParameterType::OpaquePtr)
    ...
    if let Ok(sig) = FunctionSignature::from_hex(
        "free",
        "FD 7B BD A9 FD 03 00 91 F3 0B 00 F9 F3 03 00 AA ?? 00 00 B4",
    ) {
        db.add(
            sig.with_convention(CallingConvention::Aarch64)
...
fn add_io_functions(db: &mut SignatureDatabase) {
    ...
    if let Ok(sig) =
        FunctionSignature::from_hex("exit", "FD 7B BD A9 FD 03 00 91 F3 0B 00 F9 F3 03 00 2A")
    {
        db.add(
            sig.with_convention(CallingConvention::Aarch64)
                .with_return_type(ParameterType::Void)
