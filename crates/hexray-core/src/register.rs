...
impl Register {
    ...
    pub fn name(&self) -> &'static str {
        match self.arch {
            ...
            Architecture::Amdgpu(_) => amdgpu_reg_name(self.id),
            _ => "unknown",
        }
    }
}

/// Render an AMDGPU operand id (9-bit field width on most encodings)
/// as one of: `s{n}`, `v{n}`, `vcc_lo`, `vcc_hi`, `m0`, `exec_lo`,
/// `exec_hi`, signed inline constants `0..64` and `-1..-16`, common
/// hex inline constants (`0.5`, `1.0`, `2.0`, `4.0`, `-0.5`, `-1.0`,
/// `-2.0`, `-4.0`), or `op:0xNNN` for unrecognised IDs.
///
/// Source: AMD ISA reference (Vega + RDNA), table "VOPSrc Operand
/// Encoding". M10.4 documented the same id layout in
/// `hexray-disasm/src/amdgpu/registers.rs`; this helper needs to
/// return `&'static str` to fit the `Register::name()` signature, so
/// we use a flat table.
fn amdgpu_reg_name(id: u16) -> &'static str {
    match id {
        // SGPRs s0..s101 (102 entries)
        n if n < AMDGPU_SGPR_NAMES.len() as u16 => AMDGPU_SGPR_NAMES[n as usize],
        106 => "vcc_lo",
        107 => "vcc_hi",
        124 => "m0",
        126 => "exec_lo",
        127 => "exec_hi",
        // Signed inline constants 0..=64 → ids 128..=192
        128 => "0",
        129 => "1",
        130 => "2",
        131 => "3",
        ...
        135 => "7",
        136 => "8",
        // Negative signed inline constants -1..-16 → ids 193..=208
        193 => "-1",
        194 => "-2",
        195 => "-3",
        196 => "-4",
        // Hex inline constants
        240 => "0.5",
        241 => "-0.5",
        242 => "1.0",
        243 => "-1.0",
        ...
        247 => "-4.0",
        248 => "1.0/(2*PI)",
        // VGPRs v0..v255 → ids 256..=511
        n if n >= 256 && n < 256 + AMDGPU_VGPR_NAMES.len() as u16 => {
            AMDGPU_VGPR_NAMES[(n - 256) as usize]
        }
        _ => "amdgpu_op?",
    }
}

/// Static `s0`..`s101` table for AMDGPU SGPRs.
const AMDGPU_SGPR_NAMES: &[&str] = &[
...
/// Static `v0`..`v255` table for AMDGPU VGPRs.
const AMDGPU_VGPR_NAMES: &[&str] = &[
