...
#[test]
fn test_signature_quality_smoke_from_fixtures() {
    let suite = create_standard_suite().with_config(BenchmarkConfig {
        categories: vec!["signature_quality".to_string()],
    ...
    assert_eq!(
        results.failed,
        0,
        "signature quality smoke failed:\n{}",
        results.report()
    );
    assert_eq!(
        results.passed, 3,
        "expected exactly 3 signature quality cases to run"
    );
}
