...
#[test]
fn test_variable_naming_smoke_from_fixtures() {
    let suite = create_standard_suite().with_config(BenchmarkConfig {
        categories: vec!["variable_naming".to_string()],
    ...
    assert_eq!(
        results.failed,
        0,
        "variable naming smoke failed:\n{}",
        results.report()
    );
    assert_eq!(
        results.passed, 4,
        "expected exactly 4 variable naming cases to run"
    );
}
