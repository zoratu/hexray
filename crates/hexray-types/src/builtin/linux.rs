...
pub fn load_linux_types(db: &mut TypeDatabase) {
    ...
    rusage.add_field(
        "ru_utime".to_string(),
        CType::Named("struct timeval".to_string()),
    );
    rusage.add_field(
        "ru_stime".to_string(),
        CType::Named("struct timeval".to_string()),
    ...
    utsname.add_field(
        "nodename".to_string(),
        CType::array(CType::char(), Some(65)),
    );
    ...
    sigaction.add_field(
        "sa_mask".to_string(),
        CType::array(CType::ulong(), Some(16)),
    ); // sigset_t
    ...
    db.add_function(
        FunctionPrototype::new("stat", CType::int())
            ...
            .param(
                "statbuf",
                CType::ptr(CType::Named("struct stat".to_string())),
            )
            .doc("Get file status"),
    );
    ...
    db.add_function(
        FunctionPrototype::new("bind", CType::int())
            ...
            .param(
                "addr",
                CType::ptr(CType::Named("struct sockaddr".to_string())),
            )
