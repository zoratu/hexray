...
pub fn load_macos_types(db: &mut TypeDatabase) {
    ...
    stat.add_field(
        "st_atimespec".to_string(),
        CType::Named("struct timespec".to_string()),
    ); // 32
    stat.add_field(
        "st_mtimespec".to_string(),
        CType::Named("struct timespec".to_string()),
    ); // 48
    stat.add_field(
        "st_ctimespec".to_string(),
        CType::Named("struct timespec".to_string()),
    ); // 64
    stat.add_field(
        "st_birthtimespec".to_string(),
        CType::Named("struct timespec".to_string()),
    ); // 80
    ...
    stat.add_field(
        "st_qspare".to_string(),
        CType::array(CType::longlong(), Some(2)),
    ); // 128
    stat.finalize();
    ...
    dirent.add_field(
        "d_name".to_string(),
        CType::array(CType::char(), Some(1024)),
    );
    dirent.finalize();
    ...
    db.add_typedef(
        "dispatch_queue_t",
        CType::ptr(CType::Named("dispatch_queue_s".to_string())),
    );
    db.add_typedef("dispatch_block_t", CType::ptr(CType::void()));

    // macOS-specific functions
    db.add_function(
        FunctionPrototype::new("kqueue", CType::int()).doc("Create a new kernel event queue"),
    );
    ...
    db.add_function(
        FunctionPrototype::new("kevent", CType::int())
            ...
            .param(
                "changelist",
                CType::ptr(CType::Named("struct kevent".to_string())),
            )
    ...
    db.add_function(
        FunctionPrototype::new(
            ...
            CType::typedef_ref("dispatch_queue_t"),
        )
