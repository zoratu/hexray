...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Some(Commands::Trace { action }) => {
            handle_trace_command(&binary, action)?;
        }
...
fn handle_trace_command(binary: &Binary, action: TraceAction) -> Result<()> {
    let fmt = binary.as_format();
    let arch = fmt.architecture();
...
fn handle_emulate_command(binary: &Binary, action: EmulateAction) -> Result<()> {
    use hexray_emulate::state::x86_regs;
    use hexray_emulate::Value;
