...
/// Whether `s` falls inside the reserved mangling namespace the demangler
/// recognizes, so a "this is an ordinary identifier" assertion must NOT be
/// made about it.
///
/// Mirrors the demangler's prefix logic exactly (see `normalize_prefixed_name`
/// and `demangle_rust` in the crate): it strips AT MOST ONE leading `_` before
/// checking the C++ Itanium (`_Z`) / Rust v0 (`_R`) prefixes, and treats a
/// leading `?` as MSVC. So `_Z1aa` AND its Darwin underscore-prefixed form
/// `__Z1aa` are both valid mangled names — the original guard missed the
/// `__Z` / `__R` forms, which is why proptest intermittently produced
/// `__Z1aa` (demangles to `a(signed char)`) and failed.
fn looks_potentially_mangled(s: &str) -> bool {
    s.starts_with('?')
        || s.starts_with("_Z")
        || s.starts_with("_R")
        || s.strip_prefix('_')
            .is_some_and(|core| core.starts_with("_Z") || core.starts_with("_R"))
}
...
proptest! {
    ...
    fn demangle_or_original_preserves_unmangled(s in "[a-zA-Z_][a-zA-Z0-9_]*") {
        // Only assert on identifiers OUTSIDE the reserved mangling namespace.
        // Names like `__Z1aa` are generated by this pattern but are genuine
        // mangled symbols, so demangling them is correct, not a regression.
        prop_assume!(!looks_potentially_mangled(&s));
        let result = demangle_or_original(&s);
