...
impl RiscVDisassembler {
    ...
    fn gpr(&self, id: u16) -> Register {
        ...
        Register::new(
            if self.is_64bit { Architecture::RiscV64 } else { Architecture::RiscV32 },
            class,
            id,
    ...
    fn decode_lui(&self, insn: u32, address: u64, bytes: Vec<u8>) -> Result<DecodedInstruction, DecodeError> {
        let rd = Self::rd(insn);
    ...
    fn decode_auipc(&self, insn: u32, address: u64, bytes: Vec<u8>) -> Result<DecodedInstruction, DecodeError> {
        let rd = Self::rd(insn);
        let imm = Self::imm_u(insn);
    ...
    fn decode_jal(&self, insn: u32, address: u64, bytes: Vec<u8>) -> Result<DecodedInstruction, DecodeError> {
        let rd = Self::rd(insn);
        let imm = Self::imm_j(insn);
        let target = (address as i64 + imm as i64) as u64;

        // JAL with rd=x0 is just a jump (J pseudo-instruction)
        // JAL with rd=x1 is a call
        let (mnemonic, operation, cf) = if rd == 0 {
            ("j", Operation::Jump, ControlFlow::UnconditionalBranch { target })
        } else if rd == 1 {
            ("jal", Operation::Call, ControlFlow::Call { target, return_addr: address + 4 })
        } else {
            ("jal", Operation::Call, ControlFlow::Call { target, return_addr: address + 4 })
        };
    ...
    fn decode_jalr(&self, insn: u32, address: u64, bytes: Vec<u8>) -> Result<DecodedInstruction, DecodeError> {
        let rd = Self::rd(insn);
        ...
        let (mnemonic, operation, cf, operands) = if rd == 0 && rs1 == 1 && imm == 0 {
            ...
            ("jr", Operation::Jump, ControlFlow::IndirectBranch { possible_targets: vec![] }, ops)
        } else if rd == 1 && imm == 0 {
            // JALR rd, rs1 (indirect call)
            ("jalr", Operation::Call, ControlFlow::IndirectCall { return_addr: address + 4 },
             vec![Operand::reg(self.gpr(rd)), Operand::reg(self.gpr(rs1))])
        } else {
            let ops = if imm == 0 {
                vec![Operand::reg(self.gpr(rd)), Operand::reg(self.gpr(rs1))]
            } else {
                vec![Operand::reg(self.gpr(rd)), Operand::reg(self.gpr(rs1)), Operand::imm(imm as i128, 12)]
            };
            ("jalr", Operation::Call, ControlFlow::IndirectCall { return_addr: address + 4 }, ops)
        };
    ...
    fn decode_branch(&self, insn: u32, address: u64, bytes: Vec<u8>) -> Result<DecodedInstruction, DecodeError> {
        let rs1 = Self::rs1(insn);
        ...
        let (final_mnemonic, operands) = if rs2 == 0 {
            // Comparing with zero
            match funct3 {
                0b000 => ("beqz", vec![Operand::reg(self.gpr(rs1)), Operand::pc_rel(imm as i64, target)]),
                0b001 => ("bnez", vec![Operand::reg(self.gpr(rs1)), Operand::pc_rel(imm as i64, target)]),
                0b100 => ("bltz", vec![Operand::reg(self.gpr(rs1)), Operand::pc_rel(imm as i64, target)]),
                0b101 => ("bgez", vec![Operand::reg(self.gpr(rs1)), Operand::pc_rel(imm as i64, target)]),
                _ => (mnemonic, vec![
            ...
            // Zero on left side
            match funct3 {
                0b100 => ("bgtz", vec![Operand::reg(self.gpr(rs2)), Operand::pc_rel(imm as i64, target)]),
                0b101 => ("blez", vec![Operand::reg(self.gpr(rs2)), Operand::pc_rel(imm as i64, target)]),
                _ => (mnemonic, vec![
                    Operand::reg(self.gpr(rs1)),
                    Operand::reg(self.gpr(rs2)),
                    Operand::pc_rel(imm as i64, target),
    ...
    fn decode_load(&self, insn: u32, address: u64, bytes: Vec<u8>) -> Result<DecodedInstruction, DecodeError> {
        let rd = Self::rd(insn);
        let rs1 = Self::rs1(insn);
        let imm = Self::imm_i(insn);
        let funct3 = Self::funct3(insn);
    ...
    fn decode_store(&self, insn: u32, address: u64, bytes: Vec<u8>) -> Result<DecodedInstruction, DecodeError> {
        let rs1 = Self::rs1(insn);
        let rs2 = Self::rs2(insn);
    ...
    fn decode_op_imm(&self, insn: u32, address: u64, bytes: Vec<u8>) -> Result<DecodedInstruction, DecodeError> {
        let rd = Self::rd(insn);
        let rs1 = Self::rs1(insn);
        let imm = Self::imm_i(insn);
        let funct3 = Self::funct3(insn);
        ...
        let (final_mnemonic, operands) = if mnemonic == "li" {
            ("li", vec![Operand::reg(self.gpr(rd)), Operand::imm(imm as i128, 12)])
        } else if mnemonic == "mv" {
            ("mv", vec![Operand::reg(self.gpr(rd)), Operand::reg(self.gpr(rs1))])
        } else if mnemonic == "seqz" {
            ("seqz", vec![Operand::reg(self.gpr(rd)), Operand::reg(self.gpr(rs1))])
        } else if use_shamt {
            (mnemonic, vec![
                Operand::reg(self.gpr(rd)),
                Operand::reg(self.gpr(rs1)),
    ...
    fn decode_op_reg(&self, insn: u32, address: u64, bytes: Vec<u8>) -> Result<DecodedInstruction, DecodeError> {
        let rd = Self::rd(insn);
        let rs1 = Self::rs1(insn);
        let rs2 = Self::rs2(insn);
        let funct3 = Self::funct3(insn);
        ...
        let operands = if mnemonic == "neg" {
            vec![Operand::reg(self.gpr(rd)), Operand::reg(self.gpr(rs2))]
        } else if mnemonic == "sgtz" || mnemonic == "snez" {
            vec![Operand::reg(self.gpr(rd)), Operand::reg(self.gpr(rs2))]
        } else {
            vec![
                Operand::reg(self.gpr(rd)),
    ...
    fn decode_op_imm32(&self, insn: u32, address: u64, bytes: Vec<u8>) -> Result<DecodedInstruction, DecodeError> {
        let rd = Self::rd(insn);
        let rs1 = Self::rs1(insn);
        let imm = Self::imm_i(insn);
        let funct3 = Self::funct3(insn);
    ...
    fn decode_op_reg32(&self, insn: u32, address: u64, bytes: Vec<u8>) -> Result<DecodedInstruction, DecodeError> {
        let rd = Self::rd(insn);
        let rs1 = Self::rs1(insn);
        let rs2 = Self::rs2(insn);
        let funct3 = Self::funct3(insn);
    ...
    fn decode_m_extension(&self, insn: u32, address: u64, bytes: Vec<u8>, is_32bit: bool) -> Result<DecodedInstruction, DecodeError> {
        let rd = Self::rd(insn);
        let rs1 = Self::rs1(insn);
        let rs2 = Self::rs2(insn);
        let funct3 = Self::funct3(insn);
    ...
    fn decode_system(&self, insn: u32, address: u64, bytes: Vec<u8>) -> Result<DecodedInstruction, DecodeError> {
        let rd = Self::rd(insn);
        let rs1 = Self::rs1(insn);
        let funct3 = Self::funct3(insn);
        ...
        if funct3 == 0 {
            ...
            match csr {
                ...
                0x105 => {
                    let inst = Instruction::new(address, 4, bytes, "wfi")
    ...
    fn decode_fence(&self, insn: u32, address: u64, bytes: Vec<u8>) -> Result<DecodedInstruction, DecodeError> {
        let funct3 = Self::funct3(insn);
    ...
    fn decode_amo(&self, insn: u32, address: u64, bytes: Vec<u8>) -> Result<DecodedInstruction, DecodeError> {
        let rd = Self::rd(insn);
        let rs1 = Self::rs1(insn);
        let rs2 = Self::rs2(insn);
        let funct3 = Self::funct3(insn);
    ...
    fn decode_compressed(&self, insn: u16, address: u64) -> Result<DecodedInstruction, DecodeError> {
    ...
    fn decode_c0(&self, insn: u16, address: u64, bytes: Vec<u8>, funct3: u8) -> Result<DecodedInstruction, DecodeError> {
        match funct3 {
            0b000 => {
                // C.ADDI4SPN: addi rd', sp, nzuimm
                let rd = self.c_reg(((insn >> 2) & 0x7) as u16);
                ...
                Ok(DecodedInstruction { instruction: inst, size: 2 })
            }
            0b001 => {
                // C.FLD (RV32/64) or C.LQ (RV128) - floating point load double
                let rd = self.c_reg(((insn >> 2) & 0x7) as u16);
                let rs1 = self.c_reg(((insn >> 7) & 0x7) as u16);
            ...
            0b101 => {
                ...
                let rs2 = self.c_reg(((insn >> 2) & 0x7) as u16);
                let rs1 = self.c_reg(((insn >> 7) & 0x7) as u16);
                let uimm = (((insn >> 10) & 0x7) << 3) | (((insn >> 5) & 0x3) << 6);
    ...
    fn decode_c1(&self, insn: u16, address: u64, bytes: Vec<u8>, funct3: u8) -> Result<DecodedInstruction, DecodeError> {
        match funct3 {
            0b000 => {
                ...
                let imm = ((imm << 26) as i32) >> 26; // sign extend from 6 bits

                if rd == 0 {
                    let inst = Instruction::new(address, 2, bytes, "c.nop")
                        .with_operation(Operation::Nop);
                    Ok(DecodedInstruction { instruction: inst, size: 2 })
                } else {
            ...
            0b001 => {
                if self.is_64bit {
                    ...
                    let inst = Instruction::new(address, 2, bytes, "c.jal")
                        ...
                        .with_control_flow(ControlFlow::Call { target, return_addr: address + 2 });
                    Ok(DecodedInstruction { instruction: inst, size: 2 })
                }
            }
            0b010 => {
            ...
            0b011 => {
                ...
                if rd == 2 {
                    ...
                    let imm = ((imm << 22) as i32) >> 22; // sign extend from 10 bits
                    ...
                    let imm = ((imm << 14) as i32) >> 14; // sign extend from 18 bits
            ...
            0b100 => {
                ...
                let rd = self.c_reg(((insn >> 7) & 0x7) as u16);
    ...
    fn decode_c2(&self, insn: u16, address: u64, bytes: Vec<u8>, funct3: u8) -> Result<DecodedInstruction, DecodeError> {
        match funct3 {
            0b000 => {
                ...
                if bit12 == 0 {
                    if rs2 == 0 {
                        ...
                        let inst = Instruction::new(address, 2, bytes, "c.jr")
                            ...
                            .with_control_flow(ControlFlow::IndirectBranch { possible_targets: vec![] });
                        Ok(DecodedInstruction { instruction: inst, size: 2 })
                    } else {
                        // C.MV: add rd, x0, rs2
                        if rd == 0 {
                    ...
                    if rs2 == 0 && rd == 0 {
                        ...
                        if rd == 1 {
                            ...
                            let inst = Instruction::new(address, 2, bytes, "c.jalr")
                                .with_operation(Operation::Call)
                                .with_operands(vec![Operand::reg(self.gpr(rd))])
                                .with_control_flow(ControlFlow::IndirectCall { return_addr: address + 2 });
                            Ok(DecodedInstruction { instruction: inst, size: 2 })
                        } else {
                            let inst = Instruction::new(address, 2, bytes, "c.jalr")
                                .with_operation(Operation::Call)
    ...
    fn decode_c_j_imm(&self, insn: u16) -> i32 {
        ...
        let imm = (imm11 << 11) | (imm10 << 10) | (imm9_8 << 8) | (imm7 << 7)
...
mod tests {
    ...
    fn test_addi() {
        ...
        let insn: u32 = (42 << 20) | (0 << 15) | (0b000 << 12) | (1 << 7) | 0b0010011;
    ...
    fn test_add() {
        ...
        let insn: u32 = (0 << 25) | (2 << 20) | (1 << 15) | (0b000 << 12) | (3 << 7) | 0b0110011;
        let bytes = insn.to_le_bytes();
        let result = disasm.decode_instruction(&bytes, 0x1000).unwrap();
    ...
    fn test_c_addi() {
        ...
        let insn: u16 = (0b000 << 13) | (0 << 12) | (10 << 7) | (5 << 2) | 0b01;
        let bytes = insn.to_le_bytes();
        let result = disasm.decode_instruction(&bytes, 0x1000).unwrap();
    ...
    fn test_c_li() {
        ...
        let insn: u16 = (0b010 << 13) | (0 << 12) | (10 << 7) | (10 << 2) | 0b01;
        let bytes = insn.to_le_bytes();
        let result = disasm.decode_instruction(&bytes, 0x1000).unwrap();
    ...
    fn test_c_lwsp() {
        ...
        let insn: u16 = (0b010 << 13) | (0 << 12) | (10 << 7) | (0 << 4) | (0 << 2) | 0b10;
        let bytes = insn.to_le_bytes();
        let result = disasm.decode_instruction(&bytes, 0x1000).unwrap();
    ...
    fn test_c_swsp() {
        ...
        let insn: u16 = (0b110 << 13) | (0 << 9) | (0 << 7) | (10 << 2) | 0b10;
        let bytes = insn.to_le_bytes();
        let result = disasm.decode_instruction(&bytes, 0x1000).unwrap();
    ...
    fn test_c_lw() {
        ...
        let insn: u16 = (0b010 << 13) | (0 << 10) | (0 << 7) | (0 << 6) | (0 << 5) | (0 << 2) | 0b00;
        let bytes = insn.to_le_bytes();
        let result = disasm.decode_instruction(&bytes, 0x1000).unwrap();
    ...
    fn test_c_sw() {
        ...
        let insn: u16 = (0b110 << 13) | (0 << 10) | (0 << 7) | (0 << 6) | (0 << 5) | (0 << 2) | 0b00;
        let bytes = insn.to_le_bytes();
        let result = disasm.decode_instruction(&bytes, 0x1000).unwrap();
    ...
    fn test_c_beqz() {
        ...
        let insn: u16 = (0b110 << 13) | (0 << 10) | (0 << 7) | (0 << 5) | (0 << 3) | (0 << 2) | 0b01;
        let bytes = insn.to_le_bytes();
        let result = disasm.decode_instruction(&bytes, 0x1000).unwrap();
    ...
    fn test_c_bnez() {
        ...
        let insn: u16 = (0b111 << 13) | (0 << 10) | (0 << 7) | (0 << 5) | (0 << 3) | (0 << 2) | 0b01;
        let bytes = insn.to_le_bytes();
        let result = disasm.decode_instruction(&bytes, 0x1000).unwrap();
    ...
    fn test_c_mv() {
        ...
        let insn: u16 = (0b100 << 13) | (0 << 12) | (10 << 7) | (11 << 2) | 0b10;
        let bytes = insn.to_le_bytes();
        let result = disasm.decode_instruction(&bytes, 0x1000).unwrap();
    ...
    fn test_c_jr() {
        ...
        let insn: u16 = (0b100 << 13) | (0 << 12) | (10 << 7) | (0 << 2) | 0b10;
        let bytes = insn.to_le_bytes();
        let result = disasm.decode_instruction(&bytes, 0x1000).unwrap();
    ...
    fn test_c_jalr() {
        ...
        let insn: u16 = (0b100 << 13) | (1 << 12) | (10 << 7) | (0 << 2) | 0b10;
        let bytes = insn.to_le_bytes();
        let result = disasm.decode_instruction(&bytes, 0x1000).unwrap();
    ...
    fn test_c_slli() {
        ...
        let insn: u16 = (0b000 << 13) | (0 << 12) | (10 << 7) | (5 << 2) | 0b10;
        let bytes = insn.to_le_bytes();
        let result = disasm.decode_instruction(&bytes, 0x1000).unwrap();
    ...
    fn test_c_ebreak() {
        ...
        let insn: u16 = (0b100 << 13) | (1 << 12) | (0 << 7) | (0 << 2) | 0b10;
        let bytes = insn.to_le_bytes();
        let result = disasm.decode_instruction(&bytes, 0x1000).unwrap();
    ...
    fn test_c_ldsp_rv64() {
        ...
        let insn: u16 = (0b011 << 13) | (0 << 12) | (10 << 7) | (0 << 5) | (0 << 2) | 0b10;
        let bytes = insn.to_le_bytes();
        let result = disasm.decode_instruction(&bytes, 0x1000).unwrap();
    ...
    fn test_c_sdsp_rv64() {
        ...
        let insn: u16 = (0b111 << 13) | (0 << 10) | (0 << 7) | (10 << 2) | 0b10;
        let bytes = insn.to_le_bytes();
        let result = disasm.decode_instruction(&bytes, 0x1000).unwrap();
